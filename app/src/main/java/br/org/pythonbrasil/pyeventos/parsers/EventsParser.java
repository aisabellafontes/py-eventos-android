...
public class EventsParser extends IterableAbstractPullParser<Event> {
	...
	private final DateFormat DATE_FORMAT = DateUtils.withBrasiliaTimeZone(new SimpleDateFormat("yyyy-MM-dd", Locale.US));

	// Calendar used to compute the events time, according to Brasilia timezone
	private final Calendar calendar = Calendar.getInstance(DateUtils.getBrasiliaTimeZone(), Locale.US);
